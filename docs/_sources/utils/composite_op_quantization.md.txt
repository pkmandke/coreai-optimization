...
Now in order to convert the finalized model into a coreai graph (`aimodel`):

- Firstly the composite op sub-module bodies need to be exported as torch exported programs in order to be included in the graph as externalized function calls.
- Secondly, the _patches_ added to the original `torch` model by the `_patch_model_for_externalization` API need to be restored.

The `_subexport_and_restore` takes in the **original torch model** and provides exactly these two functionalities in that order.
